...
from httpx import Auth, Request, Response, Client as _Client, USE_CLIENT_DEFAULT

if TYPE_CHECKING:
    from httpx._types import TimeoutTypes

    from .types.api import *
    from .types.errors import ErrorAttributes, Error
...
class GetPage(Protocol, Generic[T]):
    def __call__(self, **kwargs: "Unpack[PageParams]") -> tuple[list[T], int]: ...
...
class BaseClient:
    ...
    def _iter_paginated(
        ...
        while offset < total:
            items, total = func(limit=limit, offset=offset, **kwargs)
            offset += len(items)
